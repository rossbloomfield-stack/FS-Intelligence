...
export async function getIngestionOperationsStatus(): Promise<IngestionOperationsStatus> {
  ...
  const [runsResult, itemsResult, approvedResult, passagesResult, targetsResult] = await Promise.all([
    db.from("source_ingestion_runs").select("status").limit(5_000),
    db
      .from("source_items")
      .select("id,title,canonical_url,content_type,publication_date,reference_targets(reference_key,title)")
      .eq("approved", false)
      .eq("fetch_status", "parsed")
      .order("created_at", { ascending: false })
      .limit(50),
    db.from("source_items").select("id", { count: "exact", head: true }).eq("approved", true),
    db.from("source_chunks").select("id", { count: "exact", head: true }),
    db.from("reference_targets").select("id", { count: "exact", head: true }).eq("enabled", true),
  ]);
  if (runsResult.error || itemsResult.error) {
    throw new Error(`Could not load ingestion operations: ${runsResult.error?.message ?? itemsResult.error?.message}`);
  }
  const statusCounts = Object.fromEntries(
    ["queued", "running", "completed", "partial", "failed", "blocked"].map((status) => [
      status,
