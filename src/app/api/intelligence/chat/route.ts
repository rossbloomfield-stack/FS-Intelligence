...
export async function POST(request:Request){
 ...
 const retrieval=retrieveFinancialIntelligence(question,plan,sourceRows??[]);
 ...
 await supabase.from("conversations").upsert({id:conversationId,user_id:user.id,title,status:"active",context:{queryPlan:plan,freshnessAssessment:retrieval.freshnessAssessment,gaps},updated_at:new Date().toISOString()},{onConflict:"id"});
