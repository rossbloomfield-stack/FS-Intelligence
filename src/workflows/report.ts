...
export async function reportWorkflow(input: ReportWorkflowInput) {
  ...
  const qa = await qualityCheck(
    { verified, scored, briefing },
    input.analysisModel,
  );
  if (!qa.output.passed) {
    throw new FatalError(
      `QA blocked publication: ${qa.output.criticalIssues.join("; ")}`,
    );
  }

  if (!input.autoPublish) {
    await setStatus(input.reportRunId, "awaiting_approval");
    using approval = createHook<{ approved: boolean }>({
      token: `report-approval:${input.reportRunId}`,
    });
    const decision = await approval;
    if (!decision.approved) {
      throw new FatalError("Report rejected by administrator");
    }
  }

  await setStatus(input.reportRunId, "publishing");
  await publish(input.reportRunId, briefing);
  await setStatus(input.reportRunId, "completed");
  return { reportRunId: input.reportRunId, status: "completed" as const };
}
