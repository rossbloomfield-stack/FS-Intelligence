...
export function IntelligenceChat({conversationId,noteworthySignals=[]}:{conversationId:string;noteworthySignals?:NoteworthySignal[]}){
...
function NoteworthySignals({signals,onAsk}:{signals:NoteworthySignal[];onAsk:(question:string)=>Promise<void>}){if(!signals.length)return null;return <section className="prompt-suggestions" aria-labelledby="noteworthy-signals"><div className="prompt-heading"><span id="noteworthy-signals">Noteworthy market signals</span><small>Ranked by evidence and strategic relevance</small></div><div className="prompt-grid">{signals.map(signal=><button className="prompt-card prompt-card-teal" type="button" key={signal.id} onClick={()=>void onAsk(`What does this signal mean, and how strong is the evidence: ${signal.title}?`)}><span className="prompt-copy"><small>{confidenceLabel(signal.confidenceScore)} confidence · {signal.observationCount} observations · {signal.independentSourceCount} independent sources</small>{signal.title}</span><span className="prompt-arrow" aria-hidden="true"><ArrowRight size={17}/></span></button>)}</div></section>}
...
type NoteworthySignal={id:string;title:string;summary:string;confidenceScore:number;signalScore:number;independentSourceCount:number;observationCount:number};
